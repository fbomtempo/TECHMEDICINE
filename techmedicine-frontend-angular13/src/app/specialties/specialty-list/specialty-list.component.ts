...
export class SpecialtyListComponent implements OnInit, OnDestroy {
  ...
  onDelete(especialidade: Specialty): void {
    this.modalService.showConfirmModal('Confirmação', 'Tem certeza que deseja remover essa especialidade?')
      .pipe(
        take(1),
        switchMap(result => result ? this.specialtyService.delete(especialidade.id) : of())
      )
      .subscribe({
  ...
  onShowData(especialidades: Specialty[]): Specialty[] {
    if (!this.filter || this.filter == '') {
      return especialidades;
    }
    return especialidades.filter(v => {
      if (v.description.toLowerCase().indexOf(this.filter.toLowerCase()) >= 0) {
        return true;
      } else {
        return false;
      }
