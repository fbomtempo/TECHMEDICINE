...
export class DiseaseListComponent implements OnInit, OnDestroy {
  ...
  onRefresh(): void | Observable<never> {
    this.diseases$ = this.diseaseService.findAll().pipe(
      catchError(() => {
  ...
  showData(diseases: Disease[]): Disease[] {
    if (!this.filter || this.filter == '') {
      return diseases;
    }
    return diseases.filter((disease: Disease) => {
      if (
        disease.description.toLowerCase().indexOf(this.filter.toLowerCase()) >=
        0
      ) {
        return true;
      } else {
    ...
    this.modalService
      ...
      .pipe(
        ...
        switchMap((confirmResult: boolean) =>
          confirmResult ? this.diseaseService.delete(disease.id) : of()
        )
      )
      .subscribe({
